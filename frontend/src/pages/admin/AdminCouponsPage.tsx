...
import { Tag, Plus, Edit2, Trash2, X, Globe } from 'lucide-react';
...
export const AdminCouponsPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    usageLimit: '', validTill: '', isActive: true, isPlatform: true,
  });
  ...
  const resetForm = () => {
    setFormData({ code: '', type: 'percentage', discountValue: '', maxDiscount: '', minOrderValue: '', usageLimit: '', validTill: '', isActive: true, isPlatform: true });
    setEditingCoupon(null);
  };

  const handleSave = async () => {
    if (!formData.code || !formData.discountValue || !formData.usageLimit || !formData.validTill) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => { resetForm(); setShowForm(true); }}
          className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-xl font-bold shadow-sm transition-colors">
          <Plus className="h-5 w-5" /> Create Global Coupon
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold">{editingCoupon ? 'Edit Coupon' : 'Create Global Coupon'}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.code}
                  ...
                  className="w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:outline-none uppercase"
                  placeholder="FRESHAPP20" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Discount Type</label>
                  <select value={formData.type} onChange={(e) => setFormData(p => ({ ...p, type: e.target.value as any }))}
                    className="w-full px-4 py-2 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary focus:outline-none">
              ...
              <div className="flex gap-3 pt-4">
                <button onClick={handleSave}
                  className="flex-1 py-2.5 bg-primary text-primary-foreground rounded-xl font-bold hover:bg-primary/90 transition-colors">
                  ...
                  <tr key={coupon.id} className="hover:bg-gray-50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Tag className={`h-4 w-4 ${coupon.isActive ? 'text-blue-500' : 'text-gray-400'}`} />
                    ...
                    <td className="px-6 py-4">
                      {coupon.isPlatform ? (
                        <span className="text-xs font-bold bg-blue-50 text-blue-700 px-2 py-1 rounded flex items-center gap-1 w-fit">
                          <Globe className="h-3 w-3" /> PLATFORM
                        </span>
                      ) : (
                        <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">VENDOR</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => { setEditingCoupon(coupon); setFormData({ code: coupon.code, type: coupon.type, discountValue: coupon.discountValue.toString(), maxDiscount: coupon.maxDiscount?.toString() || '', minOrderValue: coupon.minOrderValue?.toString() || '', usageLimit: coupon.usageLimit.toString(), validTill: coupon.validTill, isActive: coupon.isActive, isPlatform: coupon.isPlatform }); setShowForm(true); }}
